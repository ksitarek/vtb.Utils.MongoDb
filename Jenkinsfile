...
pipeline {
    ...
    environment {
        SONARQUBE_PROJECT_KEY = "vtb-utils-mongodb"
    ...
    stages {
        ...
        stage('Publish') {
            steps { 
                ...
                sh 'dotnet pack vtb.Utils.MongoDb/vtb.Utils.MongoDb.csproj --no-build --no-restore -p:PackageVersion=${VERSION} -o . -c Release'
